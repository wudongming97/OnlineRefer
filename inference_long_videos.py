'''
Inference code for OnlineRefer, on your own videos.
...
def sub_processor(lock, pid, args, data, save_path_prefix, save_visualize_path_prefix, img_folder, video_list):
    ...
    for video in video_list[:1]:
        ...
        num_expressions = len(expression_list[:1])
        ...
        for i in range(num_expressions):
            img_folder = args.video_path
            video_name = args.video_name
            video = video_name
            exp = args.expression
            exp_id = '0'
            frames = sorted(os.listdir(os.path.join(img_folder, video_name)))
            ...
            for clip in clip_list:
                ...
                for t in range(video_len):
                    frame = frames[t]
                    # img_path = os.path.join(img_folder, video_name, frame +
                    img_path = os.path.join(img_folder, video_name, frame)
                ...
                all_pred_init_points = pred_init_points[range(video_len)][0]
                all_pred_masks = pred_masks

                save_visualize_path_prefix = args.visualize_path
                if args.visualize:
                    for t, frame in enumerate(frames):
                        ...
                        # draw init point
                        # draw_init_points(draw, all_pred_init_points, source_img.size,
                        #                  color_list[-1], source_img)
                        # draw_init_points(draw, all_pred_init_points, source_img.size,
                        #                  our_colors[-1], source_img)

                        # draw boxes, default color = color_list[i % len(color_list)]
                        # xmin, ymin, xmax, ymax = draw_boxes[0]
                        # draw.rectangle(((xmin, ymin), (xmax, ymax)), outline=tuple(our_colors[0]),
                        #                width=2)
                ...
                # save binary image
                save_path_prefix = args.mask_path
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_path', type=str, default='/data/Dataset/ref-long-videos')
    parser.add_argument('--video_name', type=str, default='89b01ce6d8')
    parser.add_argument('--expression', type=str, default='an adult tiger in the middle of two small tigers')
    parser.add_argument('--visualize_path', type=str, default='./my_own_videos_with_mask')
    parser.add_argument('--mask_path', type=str, default='./my_own_videos_masks')
